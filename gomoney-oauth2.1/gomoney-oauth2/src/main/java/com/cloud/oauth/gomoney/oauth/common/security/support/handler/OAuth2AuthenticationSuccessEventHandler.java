...
import com.cloud.oauth.gomoney.core.constant.CommonConstants;
...
public class OAuth2AuthenticationSuccessEventHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (MapUtil.isNotEmpty(map)) {
			// 发送异步日志事件
			GoUser userInfo = (GoUser) map.get(SecurityConstants.DETAILS_USER);
			log.info("用户：{} 登录成功", userInfo.getName());
			SecurityContextHolder.getContext().setAuthentication(accessTokenAuthentication);

			String startTimeStr = request.getHeader("REQUEST-START-TIME");
